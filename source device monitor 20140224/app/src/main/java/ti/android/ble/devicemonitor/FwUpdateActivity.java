...
public class FwUpdateActivity extends Activity {
	...
	private static final int RESTART_COUNT_INTERVAL = 1000;
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (BluetoothLeService.ACTION_DATA_NOTIFY.equals(action)) {
				...
				if (status != BluetoothGatt.GATT_SUCCESS) {
					...
					Toast.makeText(context, "GATT error: status=" + status,
	...
	private void stopProgramming() {
		...
		if (mProgInfo.iBlocks == mProgInfo.nBlocks) {
			...
			mTimer = new Timer();
			mTimerTask = new RestartCountdownTask();
			mTimer.scheduleAtFixedRate(mTimerTask, 500, RESTART_COUNT_INTERVAL);
		} else {
	...
	private void displayImageInfo(TextView v, ImgHdr h) {
		int imgVer = (h.ver) >> 1;
		int imgSize = h.len * 4;
		String s = String.format("Type: %c Ver.: %d Size: %d", h.imgType, imgVer,
		    imgSize);
	...
	private class RestartCountdownTask extends TimerTask {
		int count = 20;
		...
		public void run() {
			...
			runOnUiThread(new Runnable() {
				public void run() {
					mLog.setText("The device will restart in " + count + " seconds");
				}
			});
		}
	}
